package com.mantz_it.rfanalyzer;

import static java.lang.Math.abs;
import static java.lang.Math.acos;
import static java.lang.Math.cos;

import android.util.Log;

import java.util.concurrent.ArrayBlockingQueue;
import java.util.concurrent.TimeUnit;

/**
 * <h1>RF Analyzer - Analyzer Processing Loop</h1>
 *
 * Module:      AnalyzerProcessingLoop.java
 * Description: This Thread will fetch samples from the incoming queue (provided by the scheduler),
 *              do the signal processing (fft) and then forward the result to the AnalyzerSurface at a
 *              fixed rate. It stabilises the rate at which the fft is generated to give the
 *              waterfall display a linear time scale.
 *
 * Extended: Accept an optional second input queue so that paired frames (primary+secondary)
 *           can be provided to doProcessing() for cross-correlation / phase analysis.
 *
 */
public class AnalyzerProcessingLoop extends Thread {
    private int fftSize = 0;					// Size of the FFT
    private int frameRate = 10;					// Frames per Second
    private double load = 0;					// Time_for_processing_and_drawing / Time_per_Frame
    private boolean dynamicFrameRate = true;	// Turns on and off the automatic frame rate control
    private boolean stopRequested = true;		// Will stop the thread when set to true
    private float[] mag = null;					// Magnitude of the frequency spectrum
    public double instant_maximum;                      // Alex: Max signal level

    private static final String LOGTAG = "AnalyzerProcessingLoop";
    private static final int MAX_FRAMERATE = 30;		// Upper limit for the automatic frame rate control
    private static final double LOW_THRESHOLD = 0.65;	// at every load value below this threshold we increase the frame rate
    private static final double HIGH_THRESHOLD = 0.85;	// at every load value above this threshold we decrease the frame rate

    private final MainActivity activity; //Alex

    private AnalyzerSurface view;
    private FFT fftBlock = null;
    private ArrayBlockingQueue<SamplePacket> inputQueue = null;		// queue that delivers sample packets (primary)
    private ArrayBlockingQueue<SamplePacket> returnQueue = null;	// queue to return unused buffers (primary)

    // Optional secondary queues (for sdr[1]). If non-null, the run() loop will wait for paired frames.
    private volatile ArrayBlockingQueue<SamplePacket> secondaryInputQueue = null;	// queue that delivers sample packets for secondary receiver
    private volatile ArrayBlockingQueue<SamplePacket> secondaryReturnQueue = null;	// queue to return unused buffers for secondary receiver

    /**
     * Constructor. Will initialize the member attributes.
     *
     * @param view			reference to the AnalyzerSurface for drawing
     * @param fftSize		Size of the FFT
     * @param inputQueue	queue that delivers sample packets
     * @param returnQueue	queue to return unused buffers
     */
    public AnalyzerProcessingLoop(AnalyzerSurface view, int fftSize, MainActivity activity, //Alex
                                  ArrayBlockingQueue<SamplePacket> inputQueue, ArrayBlockingQueue<SamplePacket> returnQueue) {
        this.view = view;
        this.activity = activity; //Alex

        // Check if fftSize is a power of 2
        int order = (int)(Math.log(fftSize) / Math.log(2));
        if(fftSize != (1<<order))
            throw new IllegalArgumentException("FFT size must be power of 2");
        this.fftSize = fftSize;

        this.fftBlock = new FFT(fftSize);
        this.mag = new float[fftSize];
        this.inputQueue = inputQueue;
        this.returnQueue = returnQueue;
    }

    public int getFrameRate() {
        return frameRate;
    }

    public void setFrameRate(int frameRate) {
        this.frameRate = frameRate;
    }

    public boolean isDynamicFrameRate() {
        return dynamicFrameRate;
    }

    public void setDynamicFrameRate(boolean dynamicFrameRate) {
        this.dynamicFrameRate = dynamicFrameRate;
    }

    public int getFftSize() { return fftSize; }

    /**
     * Setter for optional secondary queues (sdr[1]).
     * Can be called anytime; the run loop checks these fields each iteration.
     */
    public void setSecondaryQueues(ArrayBlockingQueue<SamplePacket> secondaryIn, ArrayBlockingQueue<SamplePacket> secondaryReturn) {
        this.secondaryInputQueue = secondaryIn;
        this.secondaryReturnQueue = secondaryReturn;
    }

    /**
     * Will start the processing loop
     */
    @Override
    public void start() {
        this.stopRequested = false;
        super.start();
    }

    /**
     * Will set the stopRequested flag so that the processing loop will terminate
     */
    public void stopLoop() {
        this.stopRequested = true;
    }

    /**
     * @return true if loop is running; false if not.
     */
    public boolean isRunning() {
        return !stopRequested;
    }

    @Override
    public void run() {
        Log.i(LOGTAG,"Processing loop started. (Thread: " + this.getName() + ")");
        long startTime;		// timestamp when signal processing is started
        long sleepTime;		// time (in ms) to sleep before the next run to meet the frame rate
        long frequency;		// center frequency of the incoming samples
        int sampleRate;		// sample rate of the incoming samples

        while(!stopRequested) {
            // store the current timestamp
            startTime = System.currentTimeMillis();

            // fetch the next samples from the primary queue (and optionally the secondary queue)
            SamplePacket samples = null;
            SamplePacket samples2 = null;

            try {
                // If no secondaryInputQueue is set, we keep the original behaviour.
                if (secondaryInputQueue == null) {
                    samples = inputQueue.poll(1000 / frameRate, TimeUnit.MILLISECONDS);
                    if (samples == null) {
                        Log.d(LOGTAG, "run: Timeout while waiting on input data. skip.");
                        continue;
                    }
                } else {
                    // We need a pair: poll both queues with a shared deadline so they belong to the same frame interval.
                    long deadline = System.currentTimeMillis() + Math.max(1, (1000 / frameRate));
                    long remaining = deadline - System.currentTimeMillis();
                    samples = inputQueue.poll(Math.max(1, remaining), TimeUnit.MILLISECONDS);
                    if (samples == null) {
                        // couldn't get primary in time: skip
                        Log.d(LOGTAG, "run: Timeout while waiting on primary input data (paired mode). skip.");
                        continue;
                    }
                    remaining = deadline - System.currentTimeMillis();
                    // Allow a short grace time to fetch the secondary frame
                    samples2 = secondaryInputQueue.poll(Math.max(0, remaining), TimeUnit.MILLISECONDS);
                    if (samples2 == null) {
                        // couldn't get the second frame paired -> return primary buffer and skip this iteration
                        // Return primary buffer:
                        returnQueue.offer(samples);
                        Log.d(LOGTAG, "run: Timeout while waiting on secondary input data (paired mode). returning primary buffer and skip.");
                        continue;
                    }
                }
            } catch (InterruptedException e) {
                Log.e(LOGTAG, "run: Interrupted while polling from input queue. stop.");
                this.stopLoop();
                break;
            }

            // At this point we either have:
            // - samples != null and samples2 == null  => single-source behavior (old)
            // - samples != null and samples2 != null  => paired behavior (new)

            frequency = samples.getFrequency();
            sampleRate = samples.getSampleRate();

            // do the signal processing:
            if (samples2 == null) {
                // original single-source processing
                this.doProcessing(samples);
            } else {
                // paired processing: primary + secondary
                this.doProcessing(samples, samples2);
            }

            // return samples to the buffer pool(s)
            // For single-source case, the doProcessing() may have used/consumed samples; we return as before.
            if (samples != null)
                returnQueue.offer(samples);
            if (samples2 != null && secondaryReturnQueue != null)
                secondaryReturnQueue.offer(samples2);

            // Push the results on the surface (still derived from the primary samples)
            view.draw(mag, frequency, sampleRate, frameRate, load);

            // Calculate the remaining time in this frame (according to the frame rate) and sleep
            // for that time:
            sleepTime = (1000/frameRate)-(System.currentTimeMillis() - startTime);
            try {
                if (sleepTime > 0) {
                    // load = processing_time / frame_duration
                    load = (System.currentTimeMillis() - startTime) / (1000.0 / frameRate);

                    // Automatic frame rate control:
                    if(dynamicFrameRate && load < LOW_THRESHOLD && frameRate < MAX_FRAMERATE)
                        frameRate++;
                    if(dynamicFrameRate && load > HIGH_THRESHOLD && frameRate > 1)
                        frameRate--;

                    //Log.d(LOGTAG,"FrameRate: " + frameRate + ";  Load: " + load + "; Sleep for " + sleepTime + "ms.");
                    sleep(sleepTime);
                }
                else {
                    // Automatic frame rate control:
                    if(dynamicFrameRate && frameRate > 1)
                        frameRate--;

                    //Log.d(LOGTAG, "Couldn't meet requested frame rate!");
                    load = 1;
                }
            } catch (Exception e) {
                Log.e(LOGTAG,"Error while calling sleep()");
            }
        }
        this.stopRequested = true;
        Log.i(LOGTAG,"Processing loop stopped. (Thread: " + this.getName() + ")");
    }

    /**
     * This method will do the signal processing (fft) on the given samples (single-source)
     *
     * @param samples	input samples for the signal processing
     */
    public void doProcessing(SamplePacket samples) {
        float[] re=samples.re(), im=samples.im();
        // Multiply the samples with a Window function:
        this.fftBlock.applyWindow(re, im);

        // Calculate the fft:
        this.fftBlock.fft(re, im);

        // Calculate the logarithmic magnitude:
        float realPower;
        float imagPower;
        int size = samples.size();
        instant_maximum = -1000;
        for (int i = 0; i < size; i++) {
            // We have to flip both sides of the fft to draw it centered on the screen:
            int targetIndex = (i+size/2) % size;

            // Calc the magnitude = log(  re^2 + im^2  )
            // note that we still have to divide re and im by the fft size
            realPower = re[i]/fftSize;
            realPower = realPower * realPower;
            imagPower = im[i]/fftSize;
            imagPower = imagPower * imagPower;
            mag[targetIndex] = (float) (10* Math.log10(Math.sqrt(realPower + imagPower)));
            if (mag[targetIndex] > instant_maximum) instant_maximum = mag[targetIndex];
        }

        activity.updateTargetParams(instant_maximum);
    }

    /**
     * Paired processing (primary + secondary). This routine computes primary mag as before
     * and then notifies MainActivity with both SamplePackets so the caller can do cross-correlation.
     *
     * @param samples  primary samples (FFT drawn from this)
     * @param samples2 secondary samples (paired)
     */
    public void doProcessing(SamplePacket samples, SamplePacket samples2) {

        // Notify the activity that a paired frame is available for cross-correlation / phase analysis.
        // The activity method is left empty by default; implement your analysis in MainActivity.onSecondarySamplesAvailable(...)
        try {
            onSecondarySamplesAvailable(samples, samples2);
        } catch (Exception e) {
            Log.e(LOGTAG, "Error while calling onSecondarySamplesAvailable: " + e.getMessage());
        }
    }

    public void onSecondarySamplesAvailable(SamplePacket samples, SamplePacket samples2) {
        // Compute FFT/mag for primary (unchanged)
        //doProcessing(samples);
        float[] re=samples.re(), im=samples.im();
        // Multiply the samples with a Window function:
        this.fftBlock.applyWindow(re, im);
        // Calculate the fft:
        this.fftBlock.fft(re, im);

        float[] re2=samples2.re(), im2=samples2.im();
        // Multiply the samples with a Window function:
        this.fftBlock.applyWindow(re2, im2);
        // Calculate the fft:
        this.fftBlock.fft(re2, im2);
        float Co, Qu;
        int size = samples.size();
        instant_maximum = 0;
        for (int i = 0; i < size; i++) {
            // We have to flip both sides of the fft to draw it centered on the screen:
            int targetIndex = (i + size / 2) % size;
            Co = re[i] * re2[i] + im[i] * im2[i];
            Qu = im[i] * re2[i] - im2[i] * re[i];
            mag[targetIndex] = (float) Math.atan(-Qu / Co);
            instant_maximum += mag[targetIndex];
            // ??? note that we still have to divide re and im by the fft size
        }
        instant_maximum *= 1/size;
        activity.updateTargetParams(instant_maximum);

    }
}